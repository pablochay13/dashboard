...
{
    public partial class sort_orders_paquetes_bd_full : MetroFramework.Forms.MetroForm
    {
        MySqlConnection con = new MySqlConnection(variables.Sentencia);

        public sort_orders_paquetes_bd_full()
        {
            InitializeComponent();
        ...
        private void sort_orders_paquetes_bd_full_Load(object sender, EventArgs e)
        {
        ...
        private void filter_Click(object sender, EventArgs e)
        {
            ...
            if (cbFechas.Checked == true)
            {
                ...
                sentencia = "SELECT * FROM sort_orders_paquetes WHERE fecha_inicio BETWEEN'" + inicio + "'AND'" + fin + "'";
            }

            if (check_destino.Checked == true)
            {
                busqueda = destino_combo.Text;
                sentencia = "SELECT * FROM sort_orders_paquetes WHERE destino LIKE '%" + busqueda + "%'";
            }
            ...
            if (check_posicion.Checked == true)
            {
                ...
                sentencia = "SELECT * FROM sort_orders_paquetes WHERE posicion LIKE '%" + busqueda + "%'";
            }

            if (check_hotel.Checked == true)
            {
                busqueda = hotel_combo.Text;
                sentencia = "SELECT * FROM sort_orders_paquetes WHERE hotel LIKE '%" + busqueda + "%'";
            }
        ...
        public void llenar()
        {
            ...
            {
                con.Open();
                MySqlDataAdapter sda = new MySqlDataAdapter("SELECT * FROM sort_orders_paquetes", con);
                DataTable dt = new DataTable();
                sda.Fill(dt);
                foreach (DataRow item in dt.Rows)
                {
