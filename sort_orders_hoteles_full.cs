...
{
    public partial class sort_orders_hoteles_full : MetroFramework.Forms.MetroForm
    {
        MySqlConnection con = new MySqlConnection(variables.Sentencia);
        public sort_orders_hoteles_full()
        {
            InitializeComponent();
        ...
        private void sort_orders_hoteles_full_Load(object sender, EventArgs e)
        {
        ...
        private void filter_Click_1(object sender, EventArgs e)
        {
            ...
            if (cbFechas.Checked == true)
            {
                ...
                sentencia = "SELECT * FROM sort_orders WHERE fecha_inicio BETWEEN'" + inicio + "'AND'" + fin + "'";
            }

            if(check_destino.Checked == true)
            {
                busqueda = destino_combo.Text;
                sentencia = "SELECT * FROM sort_orders WHERE destino LIKE '%" + busqueda + "%'";
            }

            if (check_account.Checked == true)
            {
                busqueda = account_text.Text;
                sentencia = "SELECT * FROM sort_orders WHERE account LIKE '%" + busqueda + "%'";
            }

            if (check_posicion.Checked == true)
            {
                busqueda = posicion_combo.Text;
                sentencia = "SELECT * FROM sort_orders WHERE posicion LIKE '%" + busqueda + "%'";
            }

            if (check_hotel.Checked == true)
            {
                busqueda = hotel_combo.Text;
                sentencia = "SELECT * FROM sort_orders WHERE hotel LIKE '%" + busqueda + "%'";
            }
        ...
        public void llenar()
        {
            ...
            {
                con.Open();
                MySqlDataAdapter sda = new MySqlDataAdapter("select * from sort_orders", con);
                DataTable dt = new DataTable();
                sda.Fill(dt);
                foreach (DataRow item in dt.Rows)
                {
