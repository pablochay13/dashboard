...
{
    public partial class banners_rdb_coppel : MetroFramework.Forms.MetroForm
    {
        ...
        private void programing_Click_1(object sender, EventArgs e)
        {
            try
            {
                MySqlCommand agregar = new MySqlCommand("INSERT INTO banners_rdb_coppel (`fecha_inicio` , `posicion` , `destino` , `hotel` , `account`) VALUES (?fecha_inicio , ?posicion , ?destino , ?hotel , ?account)", con);
        ...
        private void calendar_sort_bd_DoubleClick(object sender, EventArgs e)
        {
            try
            {
                ...
                string sql = "SELECT * FROM banners_rdb_coppel where fecha_inicio = '" + calendar_sort_bd.SelectedDate.Value.ToString("yyyy-MM-dd") + "'";
        ...
        private void uno_CheckStateChanged(object sender, EventArgs e)
        {
            if (this.uno.Checked == true)
            {
                ...
                MySqlCommand cmd = new MySqlCommand("SELECT * FROM banners_rdb_coppel WHERE fecha_inicio='" + fecha_final + "'AND posicion='" + posicion_combo.Text + "'AND destino='" + destino_combo.Text + "' ", con);
        ...
        private void destino_combo_SelectedIndexChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                con.Open();
                string sql = "SELECT * FROM banners_rdb_coppel WHERE posicion='" + posicion_combo.Text + "'AND destino='" + destino_combo.Text + "' ";
                MySqlCommand cmd = new MySqlCommand(sql, con);
                MySqlDataReader reader = cmd.ExecuteReader();
                while (reader.Read())
                {
        ...
        public void verificar_dispo()
        {
            try
            {
                ...
                if (uno.Checked == true)
                {
                    ...
                    string sql = "SELECT fecha, posicion, destino FROM banners_rdb_coppel where fecha_inicio = '" + fecha_final + "' and posicion = '" + posicion_combo.Text + "' and destino = '" + destino_combo.Text + "'";
        ...
        public void carga_fechas()
        {
            try
            {
                if (posicion_combo.SelectedIndex == 0)
                {
                    ...
                    con.Open();
                    string sql = "SELECT * FROM banners_rdb_coppel WHERE posicion='" + posicion_selected + "'AND destino='" + destino_combo.Text + "' ";
                    MySqlCommand cmd = new MySqlCommand(sql, con);
                    MySqlDataReader reader = cmd.ExecuteReader();
                    while (reader.Read())
                    {
